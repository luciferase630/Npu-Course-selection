...
- `advanced_boundary_v1` 不是模拟录取边界表，而是一个预算占比公式：用 `m/n` 和超额人数拟合边界参量，再按课程重要性和单课上限截断。
- 预测层显著优于旧公式缩放版：平均绝对误差 `1.213647` vs `4.580537`，覆盖率 `0.942953` vs `0.724273`，平均多投 `0.944631` vs `2.21868`。
- BA 固定选课集合回放中，新公式不再像旧公式一样破坏 S048 效用指标；pure BA 与 mix30 背景下都保持基线效用，并减少拒录浪费。
- LLM 固定背景回放中，新公式在 mix30 背景明显优于旧公式；pure BA 背景则表现为效用指标略低但豆子浪费大幅降低。因此 README 不应写“无条件全面优于”，应写“预测层全面优于，策略层多数胜出，且仍存在效用/浪费权衡”。
...
| 公式 | 测试平均绝对误差 | 覆盖率 | 平均多投 | 高拥挤误差 | 低拥挤误差 |
| --- | ---: | ---: | ---: | ---: | ---: |
...
解释：`logistic_saturation` 误差最低，但覆盖率不够“稳拿”；`advanced_boundary_v1` 通过热门课安全垫校准把覆盖率推到 90%-95% 区间，同时仍显著优于旧公式。

## BA 固定选课集合回放

| 背景市场 | 策略 | 效用指标 | 选择/录取 | 花豆 | 拒录浪费 | 录取超额 | 非边际豆 |
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: |
...
| 背景市场 | 提示词 | 效用指标 | 选择/录取 | 花豆 | 拒录浪费 | 录取超额 | 非边际豆 | HHI |
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
...
解释：advanced prompt 在 mix30 市场中同时提升效用指标和豆子效率；pure BA 市场中则把豆子浪费压低很多，但效用指标略低于当前 legacy prompt 单次结果。这个失败项说明新公式不能被宣传为无条件最优，而应被定位为“更可信的边界公式 + 更强的节制机制”。

## README 写法边界

- 可以写：旧公式有爆炸和预算截断问题；新公式在预测层显著更好；在 BA 固定回放和 mix30 LLM 回放中，新公式明显优于旧公式。
