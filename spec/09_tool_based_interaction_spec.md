...
- `check_schedule` and rejected `submit_bids` results may include `repair_suggestions.suggested_feasible_bids`.
- `repair_suggestions` is feedback only. The platform must not silently apply it; the LLM must call `submit_bids`.
