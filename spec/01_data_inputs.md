...
- `eligible` 只表示学校系统是否允许申请该教学班。medium v1 默认不建先修课或行政限制表，因此所有学生对所有教学班 `eligible=true`。
- profile 不直接过滤 eligible；专业差异由 `utility` 和课程代码要求体现。
...
- medium v1 使用完整边表。未来如果引入先修课硬门槛或行政限制，才设置 `eligible=false` 或不生成该边。
...
- 对每个学生，找到其 `profile_id` 对应的所有 `profile_requirements`，复制为学生级要求并填入 `student_id`。
