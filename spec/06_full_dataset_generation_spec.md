...
```csv
profile_id,course_code,requirement_type,requirement_priority,deadline_term
CS_2026,FND001,required,progress_blocking,current
CS_2026,MCO007,required,degree_blocking,current
SE_2026,FND001,required,progress_blocking,current
SE_2026,MCO008,required,degree_blocking,current
```
...
- 每个 profile 的 `required` course_code 数量建议为 `5-10` 门。
- 可以为每个 profile 生成少量 `strong_elective_requirement` 和 `optional_target`。
- `deadline_term` 第一版可以统一填 `current`。
...
3. 把这些 requirements 复制到 `student_course_code_requirements.csv`，并填入对应的 `student_id`。
4. 因此 CS、SE、AI、MATH 等不同 profile 的学生会拥有不同课程代码要求。
