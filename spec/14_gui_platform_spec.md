...
GUI 不直接读写私钥。LLM 配置页允许用户在本机交互式填写 `OPENAI_API_KEY`、`OPENAI_MODEL` 和可选 `OPENAI_BASE_URL`，保存到仓库根目录 `.env.local`。API key 只允许写入和清除，不允许回显；状态页只显示是否已配置 key。
...
- 提供本地配置表单：API key 密码框、model 文本框、base URL 文本框。
- API key 输入框留空表示保留已有 key；勾选“清除 key”才删除。
- `OPENAI_MODEL` 必填；`OPENAI_BASE_URL` 可空，空值表示使用官方默认 endpoint。
- 保存后写入 `.env.local` 并立即更新 GUI 进程环境变量，后续从 GUI 启动的 session/replay 继承新配置。
- 不显示 API key 原文；不允许文件预览 `.env.local`；不把 key 写入 job 日志。
...
- 表单覆盖 `session run`：
  - market、population 字符串、population file、output、run id、time points、seed、config、experiment config、experiment group、interaction mode、formula prompt、focal agent、focal student id/list/share/count、background formula share、cass policy。
- 提供 population 构造器：
...
- Strategy Visual：选择 run 和可选 student id，可视化 agent 构成、投豆分布、最拥挤课程和目标学生投豆。
...
POST /api/analysis/strategy-visual
...
- LLM agent 页面必须提示 API 调用会产生费用；GUI 可写入 `.env.local`，但不返回、不打印、不预览 key。
- `/api/llm/config` 只接受本机 loopback 请求；即使 server 绑定到非 localhost，也不能远程改 key。
