...
但这里必须补一个现实边界：**真实学生没有精确的 utility 表。** 仿真里的 utility 是为了让算法和回测可以量化比较，不是要求学生坐在选课系统前计算“这门课给我多少效用”。对学生真正可操作的核心数字，仍然是：

```text
排队比 = 当前可见投豆/排队人数 m / 课程容量 n
```

学生能做的是用 `m/n` 估计边界：低于容量很多就少投，接近满员就轻保护，明显超载才考虑加码；至于“课程价值”，现实里只能用定性排序代替，比如必修/核心课、非常想上、一般想上、可替代、纯凑学分。CASS 的启发意义主要在这里：**别把公式当精确答案，而是把 m/n 当价格信号，再用自己的粗偏好判断值不值得追。**
...
注意：这是研究模型里的说法。真实学生通常不知道自己的精确 utility，只知道一个粗糙偏好排序。所以 CASS 不能被翻译成“你去算每门课 utility”，更合适的学生版说法是：先看 `m/n`，再把课程按“必修/核心、强烈想上、一般想上、可替代”分层。
...
二看课价值：
  不要计算精确 utility，只做粗排序：
  必修/核心 > 强烈想上 > 一般想上 > 可替代
  学分低的课可以扩大覆盖，但不值得无脑追高价
