...
这个仓库研究的是投豆选课。更准确地说，它是一个**投了就消耗的拍卖机制**：每个学生有固定豆子预算，课程容量有限，大家给想选的课投豆，投豆高的人更容易录取；但豆子投出去以后，无论最后录不录，都会被消耗。
...
1. 当 $m \le n$ 时，$\sqrt{m-n}$ 没有实数意义。也就是说，低竞争课反而不好处理。
...
1. **BidFlow 沙盒**：生成合成选课市场，运行普通学生、公式学生、LLM、CASS 等策略，做固定背景 replay 和统计分析。
...
我们不再给模拟数据里的绝对 cutoff 表，因为那会误导现实学生。不同学校、不同年份、不同课程结构都会改变绝对 cutoff。
...
当 $m \le n$ 时，普通课先从 `1` 豆低价试探开始；重要课可以给少量安全垫，但不要机械套高竞争公式。
...
你不需要知道全校同学的偏好分布。那几乎不可能。你也不需要计算本项目里的 utility。现实里你只需要把课程粗略分档，然后套一个有边界的公式。
...
- $r \le 1$：普通课低价试探，别用高价表达喜欢。
- $r > 1$：开始估边界。
- $r$ 很高：先问有没有替代课，不要先 all-in。

### 第二步：算基础边界

如果 $m>n$，用上面的 $s_0$ 公式估计基础边界，再得到基础投豆。

这个数不是“保录承诺”，只是告诉你：在当前拥挤程度下，竞争者的投豆边界可能在哪个预算区间。
