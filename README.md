...
如果 `m > n`，用下面这个百分比公式估基础边界：

$$
p\approx -0.3\%+3.82\%\ln(1+d)+0.98\%\ln(1+r)+3\%
$$
...
1. **先算基础豆数**：`基础豆数 = 总预算 × p`。
2. **再乘课程重要性系数**。
3. **最后过三道闸门**：公式算出来的数、剩余预算、单课上限，三者取最小。
...
这件事有点像格雷厄姆讲“安全边际”：当只有少数人理解安全边际时，它是优势；当所有人都用同一套安全边际买入，价格本身就会被抬上去，优势会被市场吃掉。
...
所以，请把这里的公式当作一个思考框架，不要当成永远有效的秘密武器。以及，向豆子神祈祷，知道这个仓库的人少一点，知道这个公式的人也少一点。
